...
class FastPandasHasher(FastNumpyHasher):
    ...
    def save(self, obj):
        if isinstance(obj, self.np.ndarray) and obj.dtype.hasobject and obj.ndim >= 1 and prod(obj.shape) > 1000:
            try:
...
class FastMemorizedFunc(MemorizedFunc):
    ...
    def _cached_call(self, args, kwargs, shelving=False):
        ...
        if not (self._check_previous_func_code(stacklevel=4) and self.store_backend.contains_item([func_id, args_id])):
            if self._verbose > 10:
                _, name = get_func_name(self.func)
                self.warn(
